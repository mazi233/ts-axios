...
describe('defaults', () => {
  ...
  test('should use modified defaults config', () => {
    ...
    return getAjaxRequest().then(request => {
      ...
      Reflect.deleteProperty(axios.defaults, 'baseURL')
    })
  })
  
  test('should use requst config', () => {
    axios('/foo', {
  ...
  test('should use GET headers', () => {
    ...
    return getAjaxRequest().then(request => {
      ...
      Reflect.deleteProperty(axios.defaults.headers.get, 'X-CUSTOM-HEADER')
    })
  })
  ...
  test('should use POST headers', () => {
    ...
    return getAjaxRequest().then(request => {
      expect(request.requestHeaders['X-CUSTOM-HEADER']).toBe('foo')
      Reflect.deleteProperty(axios.defaults.headers.post, 'X-CUSTOM-HEADER')
    })
  })
